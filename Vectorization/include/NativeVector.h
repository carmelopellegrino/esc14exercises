...
namespace nativeVector {
  ...
  inline
  constexpr unsigned int length(V1 a) { return sizeof(V1)/sizeof(a[0]);}
  ...
  inline
  bool all(V1 a) {
    constexpr unsigned int m = (0xFFFFFFFF)>>(32-sizeof(V1)/sizeof(a[0])); 
    return mask(a)==m;
  }

  bool any(bool a) { return a;}
...
#include<ostream>
  ...
  std::ostream & operator<<(std::ostream & co, NativeVector<float> const & v) {
     co << '('<< v[0];
     for (auto i=1U; i<  nativeVector::length(v); ++i)  co <<',' << v[i];
     return co <<')';
  }
  inline
